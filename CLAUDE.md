...
- **Frontend:** Single-file vanilla HTML/CSS/JS at `app/static/index.html` (~3600 lines)
...
│   ├── task_template.py      # TaskTemplate (quick-create presets)
...
│   ├── template_api.py      # Task template CRUD + use tracking
...
mobile/                          # Flutter mobile app
  lib/
    ...
    models/                      # Task, Message, Reminder models
    services/                    # Auth, API, Task, AI, Calendar services
    screens/                     # Login, Home, Dashboard, Tasks, Chat, Calendar
    widgets/                     # TaskCard, PriorityBadge, StatusChip
  pubspec.yaml
```
...
- **i18n** — 295+ keys in `I18N` object (EN/KH), `data-i18n` attributes, `t('key')` helper function
...
## Database
PostgreSQL with tables: users, messages, tasks, reminders, task_comments, recurring_tasks, task_groups, task_subgroups, team_roles, task_actions, task_dependencies, notifications, categories, subcategories, audit_logs, push_subscriptions, email_preferences, system_settings, task_templates
...
- **Service Worker cache:** Currently at v28 — bump on every frontend change
- **Static IP:** Reserve with `bash scripts/reserve-static-ip.sh` from GCP Cloud Shell (promotes ephemeral IP to static)
...
1. Flutter mobile app - publish to Play Store / App Store
2. Advanced analytics - custom report builder, scheduled email reports
3. AI workflow automation - auto-assign tasks based on workload, smart deadlines
4. File management - direct file upload/download without Google Drive
5. Time tracking - log time per task, timesheet reportsMulti-user collaboration with real-time sync
